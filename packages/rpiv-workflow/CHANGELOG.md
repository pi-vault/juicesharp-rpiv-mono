...
### Added
- Typed dispatch builders `produces.prompt({ … })` / `acts.prompt({ … })` (mirroring `produces.script`) — narrowed options that structurally omit `skill`/`run`/`fanout`/`iterate`/`reads`, so an invalid dispatch combo fails to compile instead of only failing load validation. The bare `prompt` field stays for the runtime + programmatic embedders; load validation stays as the backstop for those paths.
- New `prompt` stage dispatch — the third dispatch alongside skill (`/skill:<name>`) and script `run`. A stage with `prompt: string | PromptFn` sends raw text straight to the model (a "chat turn") with no skill body and no skill-registry check. Orthogonal to `kind` (a `produces` prompt stage runs the `outcome` collector and publishes; a `side-effect` prompt stage just talks) and to `sessionPolicy` — `continue` + `prompt` is a follow-up turn that builds on a session a prior stage populated, the one shape skill/script dispatch couldn't express. Validated at load + preflight (mutually exclusive with explicit `skill`, `run`, `reads`, and — in v1 — `fanout`/`iterate`; `produces` + `prompt` still requires an `outcome`). New export: `PromptFn`. See the authoring guide's "Prompt stages (raw-text dispatch)" section.
